/**
 * Basic MD5 implementation for Gravatar support
 */
function md5(string: string) {
  function md5_Cycle(x: any, k: any) {
    ...
    d = ii(d, a, b, c, k[3], 10, -1894907506);
  ...
  function md5_1(s: string) {
    ...
    tail[14] = n * 8;
    md5_Cycle(state, tail);
    for (i = 0; i < 4; i++) txt += md5_Hex(state[i]);
    return txt;
  }

  function md5_Hex(n: any) {
    let s = "", j;
  ...
  function md5_Convert(s: string) {
    let words = [], i;
    for (i = 0; i < s.length; i += 4) words[i >> 2] = s.charCodeAt(i) | (s.charCodeAt(i + 1) << 8) | (s.charCodeAt(i + 2) << 16) | (s.charCodeAt(i + 3) << 24);
    return words;
  }
 ...
 */
export function getGravatarUrl(email: string | null | undefined, size: number = 100) {
  if (!email) return `https://i.pravatar.cc/${size}`;
